...
% function [K2Score,JS,LR,ME] = multi_criteriaEvaluationFuns4(snp_com,state)
%  function [K2Score,Gtest,Gini_Score,JE_Score] = multi_criteriaEvaluationFuns4(snp_com,state)
%function [K2Score,JS,LR,JE_Score,HBP] = multi_criteriaEvaluationFuns4(snp_com,state)
...
% B = abs(ebsol+(disease + 0.1)./ (0.1+ control) ).^0.2;
% BP = B./sum(B) + 1e-6;
% % case/control entropy:ratioEntropy
% HBP = - sum(BP.*log(BP));
